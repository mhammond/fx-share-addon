...
SharePanel.prototype = {
  ...
  registerAPIs: function(worker, frame) {
    ...
    worker.port.on("owa.service.oauth.call", function(args) {
      OAuthConsumer.call(args.svc, args.data, function(req) {
        ...
        let response = {status: req.status, statusText: req.statusText, responseText: req.responseText};
        worker.port.emit(args.result, response);
      });
    });

    worker.port.on("owa.service.oauth.login", function(args) {
      mediator.doOWALogin(args, function(response) {
        worker.port.emit(args.result, response);
      });
    });
  ...
  onOWALogin: function(params) {
    let self = this;
    this.doOWALogin(params, function(response) {
          self.panel.port.emit("owa.mediation.onLogin", params);
    });
  },

  doOWALogin: function(params, callback) {
