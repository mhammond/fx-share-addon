...
*/
  ...
  init: function() {
    ...
    // we act as a trusted "agent" for the service.
    let {
      SharePanel
    } = require("panel");
...
exports.main = function(options, callbacks) {
  ...
  // create a page-mod so *all* pages can use the oauth helpers.  This is
  // only necessary for our demo.
  var pageMod = require("page-mod");
  pageMod.PageMod({
    include: "*",
    contentScriptWhen: 'end',
    contentScriptFile: require("self").data.url("servicesapi.js"),
    onAttach: function onAttach(worker) {
      let {SharePanel} = require("panel");
      let wm = Cc["@mozilla.org/appshell/window-mediator;1"]
            .getService(Ci.nsIWindowMediator);
      let window = wm.getMostRecentWindow("navigator:browser");
      let panel = new SharePanel(window, {});
      panel.registerAPIs(worker, null);
    }
  });
  return;
};
